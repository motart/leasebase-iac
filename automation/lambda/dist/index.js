...
async function getSecret(name) {
    ...
    const cmd = new client_secrets_manager_1.GetSecretValueCommand({ SecretId: name });
...
async function dispatchWorkflow(repo, issueKey, projectKey, mode) {
...
async function handler(event) {
    ...
    try {
        ...
        const receivedSecret = event.headers["x-leasebase-webhook-secret"] ??
        ...
        const hasDirectMapping = !!repoMap[projectKey] ||
            (payload.issue.fields?.labels ?? []).some((l) => l.match(/^service:.+$/i));
